        ...
        if (moment(jobDate).isBefore(moment(tooLate))) {
          ...
          // if not recurring, remove the job from the Jobs database.
            ...
            User.findById(job.parent, function(err, user) {
              if (user === null) console.log('user ' + user + ' is NULL for removing job from user array.');
      ...
      newQueue.save(function(err, data) {
        ...
        User.findById(newConcierge._id, function(err, user) {
          ...
          user.save(function(err) {
            ...
            client.sendMessage({ to: user.phone, from: config.twilioNumber,
              body:'You have been successfully matched to call user: ' + data.parentName.first + ' ' + data.parentName.last + ' ' + moment(data.jobDate).fromNow()},
            function(err) {
              if (err) return console.log('concierge match sms could not be sent:  ' + err);
            });
          });
